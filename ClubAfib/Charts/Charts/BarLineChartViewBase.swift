...
//    @objc open var borderColor = NSUIColor.black
    ...
    @objc private func tapGestureRecognized(_ recognizer: NSUITapGestureRecognizer)
    {
        ...
        if recognizer.state == NSUIGestureRecognizerState.ended
        {
            ...
            {
                if isECG {
                    let pos =  self.highlighter?.getValsForTouch(x: recognizer.location(in: self).x, y: recognizer.location(in: self).y)
                    if pos != nil && h != nil {
                        if abs(Double(pos!.x) - h!.x) < 0.5 {
                            lastHighlighted = nil
