...
class BloodPressureMarkerView: BalloonMarker {
    ...
    var customLabelDelegate: CustomLabelMarkerViewDelegate?
    ...
    public override func refreshContent(entry: ChartDataEntry, highlight: Highlight) {
        ...
        if let delegate = customLabelDelegate {
            return setLabel(delegate.getLabel(entry: entry, highlight: highlight))
        }
        ...
        if let candleEntry = entry as? CandleChartDataEntry {
            strBMP = candleEntry.low == candleEntry.high ? "\(Int(candleEntry.high)) mmHg\n" : "\(Int(candleEntry.low))-\(Int(candleEntry.high)) mmHg\n"
        }
        else if let rangeBarEntry = entry as? RangeBarChartDataEntry {
            strBMP = rangeBarEntry.start == rangeBarEntry.end ? "\(Int(rangeBarEntry.end)) mmHg\n" : "\(Int(rangeBarEntry.start))-\(Int(rangeBarEntry.end)) mmHg\n"
        }
