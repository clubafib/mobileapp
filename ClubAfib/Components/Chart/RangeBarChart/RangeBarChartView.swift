...
class RangeBarChartView: BarLineChartViewBase, RangeBarChartDataProvider
{
    ...
    internal override func initialize()
    {
        ...
        renderer = RangeBarChartRenderer(dataProvider: self, animator: _animator, viewPortHandler: _viewPortHandler)
        
        self.highlighter = RangeBarHighlighter(chart: self)
    ...
    {
        guard let data = self.data as? RangeBarChartData
            else { return }
    ...
    @objc open func getBarBounds(entry e: RangeBarChartDataEntry) -> CGRect
    {
        guard let
            data = _data as? RangeBarChartData,
            let set = data.getDataSetForEntry(e) as? IRangeBarChartDataSet
    ...
    /// Groups all RangeBarDataSet objects this data object holds together by modifying the x-value of their entries.
    ...
    @objc open func groupRangeBars(fromX: Double, groupSpace: Double, barSpace: Double)
    {
        guard let rangeBarData = self.rangeBarData
            else
        {
        ...
        rangeBarData.groupBars(fromX: fromX, groupSpace: groupSpace, barSpace: barSpace)
    ...
    // MARK: - RangeBarChartProvider
    
    open var rangeBarData: RangeBarChartData?
    {
        return _data as? RangeBarChartData
    }
